        ...
        <div className="min-h-screen bg-gray-50 flex">
            ...
            <div className="w-64 bg-white border-r border-gray-200 flex flex-col hidden md:flex">
                ...
                <nav className="flex-1 p-4 space-y-2">
                    ...
                    <button onClick={() => navigate("/mentor/calendar")} className="w-full flex items-center space-x-3 px-4 py-3 rounded-lg text-gray-600 hover:bg-gray-50 transition-colors">
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                        <span className="font-medium">Calendar</span>
                    </button>

                    <button onClick={() => navigate("/mentor/sessions")} className="w-full flex items-center space-x-3 px-4 py-3 rounded-lg text-gray-600 hover:bg-gray-50 transition-colors">
                            ...
                            <div className="w-1/3 border-r border-gray-200 overflow-y-auto">
                                {conversations.length === 0 ? (
                                    <div className="p-8 text-center">
                                        ...
                                        <p className="text-gray-500 mt-1">Your session messages will appear here</p>
                                    </div>
                                ) : (
                                        ...
                                        >
                                            <div className="flex items-start space-x-3">
                                                <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center overflow-hidden flex-shrink-0">
                                                    {conv.session.userId?.photo ? (
                                                        <img src={conv.session.userId.photo} alt={conv.session.userId.name} className="w-full h-full object-cover" />
                                                    ) : (
                                                        <span className="text-white font-bold text-lg">{conv.session.userId?.name?.charAt(0) || "E"}</span>
                                                    )}
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center justify-between">
                                                        <h3 className="font-semibold text-gray-900 truncate">{conv.session.userId?.name}</h3>
                                                        {conv.unreadCount > 0 && (
                                            ...
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center space-x-3">
                                                    <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center overflow-hidden">
                                                        {selectedConversation.session.userId?.photo ? (
                                                            <img src={selectedConversation.session.userId.photo} alt={selectedConversation.session.userId.name} className="w-full h-full object-cover" />
                                                        ) : (
                                                            <span className="text-white font-bold">{selectedConversation.session.userId?.name?.charAt(0) || "E"}</span>
                                                        )}
                                                    </div>
                                                    <div>
                                                        <h2 className="font-bold text-gray-900">{selectedConversation.session.userId?.name}</h2>
                                        ...
                                        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
                                            {messages.length === 0 ? (
                                                ...
                                                messages.map((msg, idx) => {
                                                    const isOwnMessage = msg.senderType === "mentor";
                                                    return (
